...
/**
 * @author ShubhamGaur/
 * Updated by Vishal kumar on 6 Jan 2020
 *
 */
@Service
public class InjectionPlanService {
	...
	public void SaveUpdate(Integer pid, List<String> input1, String id) {
		...
		// get the row from the form input.....// 7 value in one row...
		for (int i = 0; i < input1.size(); i=i+8) {
		if(id.equalsIgnoreCase(input1.get(7+i)))
				{
			for (int j = 0; j <8; j++) {
				input.add(input1.get(i+j));
			}	}
		}
		int j = 0;
		String slurry = calcSlurry(input.get(4), input.get(5), input.get(6));
		...
		for (int i = 0; i < injectionPlanModels.size(); i++) {
			...
			if(id.equalsIgnoreCase(Integer.toString(model.getId())))
			{
	...
	j=0;	}
		...
		specialMethodToUpdatelapseTime(pid);
	...
	public String calcSlurry(String clean, String begin, String end) {
		...
		double rounded = Math. round(slurry * 100.0) / 100.0; /// rounding up to 2 digits
		 slurry =rounded;
	...
	public String calcCumSlurry(Integer pid, String slurry) {
		...
		slurry1 = Double.parseDouble(df2.format(slurry1));
	...
	public String calcStageProp(String clean, String begin, String end) {
		...
		df2.setRoundingMode(RoundingMode.DOWN);
		StageProp = Double.parseDouble(df2.format(StageProp));
	...
	public String calcStepTime(String slurry, String rate) {
		...
		df2.setRoundingMode(RoundingMode.DOWN);
		steptime = Double.parseDouble(df2.format(steptime));
	...
	public void specialMethodToUpdatelapseTime(Integer pid)
	{
		ProjectDetails detail = detailRepo.findById(pid).orElse(null);
		List<InjectionPlanModel> injectionPlanModels = planRepo.findBydetails(detail);
		double lapseTime=0.0;
		for (int i = 0; i < injectionPlanModels.size(); i++) {
		String	x=injectionPlanModels.get(i).getSteptime();
			lapseTime = lapseTime+ Double.parseDouble(x);
			df2.setRoundingMode(RoundingMode.DOWN);
			lapseTime = Double.parseDouble(df2.format(lapseTime));
		
			
			injectionPlanModels.get(i).setCumStepTime(Double.toString(lapseTime));
			planRepo.save(injectionPlanModels.get(i));
		}
