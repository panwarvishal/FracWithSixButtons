...
public class InputFile {
	...
	ProjectDetails pid=	prodetails.getOne(p_id);
		List<MiniFracModel> m= service.findBydetails(prodetails.getOne(p_id));
		if (m.size()>0)
		{service.deleteBydetails(prodetails.getOne(p_id));}
		...
		for(int i=worksheet.getFirstRowNum();i<worksheet.getLastRowNum() ;i++) {
			...
			if(row!=null)
			{
				...
				c=row.getCell(j);
			//	if(c == null || "".equals(c.getRichStringCellValue().getString()))
				  	if(c==null)
					{
						continue;
					}else {
				pressure=row.getCell(0).toString();
