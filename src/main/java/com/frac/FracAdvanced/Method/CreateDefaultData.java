...
/**
 * 
 * THis file has all single layer input which we use with model having verticle data. like ReservoirLithollogy
 * 
 * @author Vishal Kumar
 *
 */
...
public class CreateDefaultData {
	...
	@Autowired
	private SingleLayerInputRepo singlelayerRepo;
	@Autowired
	...
	try {			
		FileReader file=new FileReader(path+"/DefaultData/DefaultData1.txt");	 		
		  ...
		  br.close();
	             ...
	             public void saveSingleLayerMethod(int pid) throws Exception
	            	{
	            	             ProjectDetails pd1=	detailRepo.getOne(pid);
	            		         String[] as=null;
	            		         String path=session.getServletContext().getRealPath("/");		 
	            	try {		
	            		  FileReader file=new FileReader(path+"/DefaultData/SingleLayerInput.txt");	 		
	            		  BufferedReader br = new BufferedReader(file); 	  
	            		  String st; 
	            		  String type=null;
	            		  SingleLayerInputModel m1=null;
	            		  while ((st = br.readLine()) != null) 
	            		  {    
	            			    m1= new SingleLayerInputModel();
	            			    as = st.split("[=]");	
                                m1.setParam(as[0]);	            				  
	            			    m1.setValue(as[1]);
	            			    m1.setPid(pd1);
	            			    singlelayerRepo.save(m1);
	            			  } 
	            		  br.close();  	  
	            	} catch(Exception e) { System.out.println(e);}  }  
